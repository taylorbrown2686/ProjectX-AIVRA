...
public class ProjectileHit : MonoBehaviour
{
    ...
    private void Tm_CollisionEnter(object sender, RFX1_TransformMotion.RFX1_CollisionInfo e)
    {
        ...
        if ((soulTarget = e.Hit.transform.GetComponent<SoulTarget>() )!= null)
        {
            SoulUnit soulUnit = soulTarget.soulUnit;
            soulUnit.UseHp(damage);
            if (dps > 0)
            {
                StartCoroutine(soulUnit.HpPerSecond(-1 * dps, duration));
            }
        }
