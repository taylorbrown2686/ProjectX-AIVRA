...
public class OnlineMapsMarker3DManager : OnlineMapsMarkerManagerBase<OnlineMapsMarker3DManager, OnlineMapsMarker3D>
{
    ...
    /*public void CreatePlayerMarker() {
      if (playerMarker == null && Input.location.status == LocationServiceStatus.Running) {
        OnlineMapsMarker3D marker = Create(Input.location.lastData.longitude, Input.location.lastData.latitude, defaultPrefab);
        playerMarker = marker;
      } else if (playerMarker != null && Input.location.status == LocationServiceStatus.Running)  {
        playerMarker.latitude = Input.location.lastData.latitude;
        playerMarker.longitude = Input.location.lastData.longitude;
      }
    }

    public void CreateEnemyMarker() {
      foreach (GameObject obj in GameObject.FindGameObjectsWithTag("Enemy")) {
        PlaceAtLocation loc = obj.transform.parent.gameObject.GetComponent<PlaceAtLocation>();
        double lat = loc.Location.Latitude;
        double lng = loc.Location.Longitude;
        GameObject.Find("asdfg").GetComponent<Text>().text = lat.ToString() + " " + lng.ToString();
      }
    }*/
}
